...
export default function App() {
  ...
  return <div className="app-shell">
    <aside className="sidebar">
      ...
      <div className="workspace-switch"><span className="workspace-avatar">B</span><span><strong>내 워크스페이스</strong><small>Personal workspace</small></span><Icon name="chevron" size={16}/></div>
      ...
      <div className="sidebar-bottom"><div className="sidebar-tip"><span className="tip-icon"><Icon name="spark" size={16}/></span><strong>아이디어를 현실로</strong><p>생각을 적으면 Buildflow가 작동하는 프로그램으로 만들어 드립니다.</p></div><div className="sidebar-footer">BUILDFLOW <span>v0.1</span></div></div>
    </aside>
    ...
    <main className="main-content">
      ...
      <div className="content-wrap">
        <div className="page-intro"><div><div className="eyebrow">YOUR CREATIVE WORKSPACE</div><h1>{s.current ? s.current.name : '좋은 아이디어의 시작'}</h1><p>{s.current ? '요청을 더해 프로젝트를 발전시키고, 실행 상태를 확인하세요.' : '만들고 싶은 것을 이야기해 주세요. 나머지는 Buildflow가 이어갑니다.'}</p></div><button type="button" className="new-project-button" onClick={() => { s.setCurrent(); s.setVersions([]); setText(''); setAdvanced(false); }}><Icon name="plus" size={18}/> 새 프로젝트</button></div>

        {!s.current && <>
          <section className="hero-card"><div className="hero-copy"><div className="hero-kicker"><Icon name="spark" size={15}/> BUILD WITH AI</div><h2>아이디어만 있다면,<br/>시작할 준비는 끝났어요.</h2><p>간단한 설명 하나로 나만의 앱을 만들고, 수정하고, 실행해 보세요.</p><button type="button" onClick={() => document.getElementById('project-prompt')?.focus()}>지금 시작하기 <Icon name="arrow" size={17}/></button></div><div className="hero-art" aria-hidden="true"><div className="art-orbit orbit-one"/><div className="art-orbit orbit-two"/><div className="art-window"><div className="art-window-bar"><i/><i/><i/></div><div className="art-window-row wide"/><div className="art-window-row"/><div className="art-window-box"><Icon name="spark" size={26}/></div></div><div className="art-float float-one"><Icon name="check" size={17}/></div><div className="art-float float-two"><Icon name="code" size={18}/></div></div></section>
          <div className="section-heading"><div><h2>내 프로그램</h2><p>최근 작업한 프로그램을 한눈에 확인하세요.</p></div><span className="section-count">전체 {s.projects.length}개 · 사용 가능 {runningCount}개</span></div>
        ...
        <section className="composer-card"><div className="composer-heading"><span className="composer-icon"><Icon name="spark" size={19}/></span><div><h2>{s.current ? '어떻게 바꿔볼까요?' : '무엇을 만들어 볼까요?'}</h2><p>{s.current ? '원하는 변경 사항을 자연스럽게 설명해 주세요.' : '만들고 싶은 프로그램을 편하게 설명해 주세요.'}</p></div></div><textarea id="project-prompt" value={text} onChange={(event) => setText(event.target.value)} disabled={s.busy} placeholder={s.current ? '예: 첫 화면에 검색 기능을 추가하고 색상을 바꿔줘.' : '예: 우리 가족이 함께 쓸 수 있는 채팅 앱을 만들어줘.'}/><div className="composer-footer"><span><Icon name="spark" size={15}/> Buildflow AI가 작업을 도와드립니다</span><button type="button" className="primary-button" disabled={s.busy || !text.trim()} onClick={() => void act()}>{s.busy ? '작업 중…' : s.current ? '변경 요청하기' : '프로젝트 만들기'} {!s.busy && <Icon name="arrow" size={17}/>}</button></div></section>
